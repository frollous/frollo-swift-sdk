...
//  FrolloSDK
//
//  Created by Nick Dawson on 4/10/18.
...
class OTPTests: XCTestCase {
    ...
    func testInit() {
        // Create a generator
        let factor = OTP.Factor.counter(111)
        let secret = "12345678901234567890".data(using: String.Encoding.ascii)!
        let algorithm = OTP.Algorithm.sha256
        let digits = 8
        ...
        let other_factor = OTP.Factor.timer(period: 123)
        let other_secret = "09876543210987654321".data(using: String.Encoding.ascii)!
        let other_algorithm = OTP.Algorithm.sha512
        let other_digits = 7
        
        let other_generator = OTP(
            factor: other_factor,
            secret: other_secret,
    ...
    func testCounter() {
        ...
        for (timeSinceEpoch, period, count) in factors {
            ...
            let timer = OTP.Factor.timer(period: period)
            let counter = OTP.Factor.counter(count)
            let secret = "12345678901234567890".data(using: String.Encoding.ascii)!
            let hotp = OTP(factor: counter, secret: secret, algorithm: .sha1, digits: 6)
                .flatMap { try? $0.password(at: time) }
            let totp = OTP(factor: timer, secret: secret, algorithm: .sha1, digits: 6)
                .flatMap { try? $0.password(at: time) }
    ...
    func testPasswordAtInvalidTime() {
        guard let generator = OTP(
            factor: .timer(period: 30),
            secret: Data(),
            algorithm: .sha1,
            digits: 6
            ) else {
                XCTFail("Failed to initialize a OTP.")
                return
        }
    ...
    func testPasswordWithInvalidPeriod() {
        ...
        let generator = OTP(factor: .timer(period: 0), secret: Data(), algorithm: .sha1, digits: 8)
        XCTAssertNil(generator)
    }
    ...
    func testPasswordWithInvalidDigits() {
        ...
        let generator = OTP(factor: .timer(period: 30), secret: Data(), algorithm: .sha1, digits: 3)
        XCTAssertNil(generator)
    }
    ...
    func testHOTPRFCValues() {
        ...
        for (counter, expectedPassword) in expectedValues {
            let generator = OTP(factor: .counter(counter), secret: secret, algorithm: .sha1, digits: 6)
    ...
    func testTOTPRFCValues() {
        let secretKeys: [OTP.Algorithm: String] = [
        ...
        let expectedValues: [OTP.Algorithm: [String]] = [
        ...
        for (algorithm, secretKey) in secretKeys {
            ...
            let generator = OTP(factor: .timer(period: 30), secret: secret, algorithm: algorithm, digits: 8)
    ...
    func testTOTPGoogleValues() {
        ...
        for (algorithm, expectedPasswords) in expectedValues {
            let generator = OTP(factor: .timer(period: 30), secret: secret, algorithm: algorithm, digits: 6)
