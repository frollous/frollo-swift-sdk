...
class PaymentsTests: XCTestCase {
        ...
        payments.verifyPayAnyone(accountHolder: "Joe Blow", accountNumber: "98765432", bsb: "123456") { result in
            switch result {
                ...
                case .success(let response):
                    ...
                    XCTAssertEqual(response.bsbName, "Volt Bank Limited")
                    XCTAssertEqual(response.institutionMnemonic, "VOL")
                    XCTAssertEqual(response.valid, true)
                    XCTAssertNotNil(response.address)
                    expectation1.fulfill()
            }
        }
        ...
        payments.verifyPayAnyone(accountHolder: "Joe Blow", accountNumber: "98765432", bsb: "123456") { result in
            switch result {
                ...
                case .success(let response):
                    ...
                    XCTAssertEqual(response.institutionMnemonic, nil)
                    XCTAssertEqual(response.address, nil)
                    expectation1.fulfill()
            }
        }
    ...
    func testVerifyValidBPAY() {
        let expectation1 = expectation(description: "Network Request 1")

        let config = FrolloSDKConfiguration.testConfig()

        stub(condition: isHost(config.serverEndpoint.host!) && isPath("/" + PaymentsEndpoint.verifyBPAY.path)) { (request) -> HTTPStubsResponse in
            return fixture(filePath: Bundle(for: type(of: self)).path(forResource: "verify_BPAY_response", ofType: "json")!, headers: [ HTTPHeader.contentType.rawValue: "application/json"])
        }

        let payments = Payments(service: service)
        payments.verifyBPAY(billerCode: "123456", crn: "98765432") { result in
            switch result {
                case .failure(let error):
                    XCTFail(error.localizedDescription)
                case .success(let response):
                    XCTAssertEqual(response.valid, true)
                    XCTAssertEqual(response.billerName, "AGL")
                    expectation1.fulfill()
            }
        }
